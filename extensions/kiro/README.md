...
| **Kiro IDE 0.x**                           | ❌ **Not supported** | The v1 JSON hook format does not exist; the file is ignored silently |
...
The v1 JSON hooks system — a `"version": "v1"` file with a `hooks` array, a `PreToolUse` trigger, read from `.kiro/hooks/` — **shipped in Kiro IDE 1.0.0 on June 25, 2026** ([changelog](https://kiro.dev/changelog/ide/), [what's new in v1](https://kiro.dev/docs/ide/whats-new-v1/)). Pre-1.0 builds are not free of tool interception altogether — 0.9 had Pre/Post Tool Use in the legacy `.kiro.hook` agent-hooks format, which the v1 migration maps `preToolUse` → `PreToolUse` from — but they do not understand the standalone v1 JSON hook file this installer writes. They do not error on it; they ignore it. **On an 0.x IDE this gate is therefore not merely unsupported, it is invisible** — install succeeds, `--check` can pass, the Agent Hooks panel shows nothing amiss, and every tool call runs unauthorized. Porting the gate to the legacy format is not a workaround we support: it is a different file format and trigger vocabulary, and the floor below is the supported answer.
...
- **A detected pre-1.0 build is a `[FAIL]`** — it does not understand the v1 JSON hook file at all, and no install layout changes that.
- **A detected 1.0.0–1.0.181 build depends on how you installed.** Globally-only, it is a `[FAIL]`: the hook is written where those builds never look. But run `--check --workspace=<project>` after a `--workspace` install and the same build is a `[PASS]`, because workspace-scoped hooks *do* load there — the pass text spells out that the guarantee covers that workspace and no other. Failing a setup this tool itself recommends would only teach you to ignore the verdict.
- **No IDE found at any known location is a NOTE, not a failure.** A machine that only runs Kiro CLI V3 legitimately has no IDE installed, and failing there would make `--check` useless for CLI users. The final summary then says every prerequisite *that could be verified* is in place, and names the IDE version as the one it could not — an all-`[PASS]` run must never read as more assurance than it earned.

When the app is not at one of those paths, detection falls back to running `kiro --version` against a `kiro` shim on your `PATH`, and the reported condition says which mechanism answered; an IDE that *is* found on disk but whose version cannot be read is reported as a warning rather than a pass or a fail, because nothing was proven either way and you should check it by hand.

Because detection reads the app on disk, it tells you about the *installed* application, not about a running process — it cannot see a stale IDE still running an older build, and (as below) it cannot see whether a running IDE has actually loaded the hook. An IDE installed somewhere other than the paths above, with no `kiro` shim on `PATH` either, reads as "not found" and produces the NOTE, so check the version by hand if you install Kiro to a custom location.

## Known limitations

Stated plainly, because a security control that overstates its coverage is worse than one that does not exist.

- **A Kiro IDE older than 1.0.182 fails silently.** Nothing errors: the hook file is written, the install reports success, and no tool call is ever checked. Two sub-cases, both silent — a pre-1.0 IDE does not understand the v1 JSON hook file and ignores it outright, and a 1.0.0–1.0.181 IDE does understand it but never scans global `~/.kiro/hooks/`, so only a `--workspace` install registers anything. `--check` catches this **only when it can identify the IDE**, from a standard install location or from a `kiro` shim on `PATH`; a custom install path with no such shim reads as "no IDE installed" and gets a NOTE rather than a failure, and an IDE whose version cannot be read gets a warning rather than a failure. Verify the IDE version yourself before trusting an all-`[PASS]` run on a machine where you use the IDE.
