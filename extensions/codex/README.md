# Denied SDK Plugin for Codex CLI

Codex CLI can execute powerful tools — shell commands, file edits, MCP servers, and more. [Denied](https://denied.dev) defines the boundaries of what your Codex agent can and cannot do: before any tool executes, this plugin checks with the Denied authorization server whether the agent is permitted to run it. If the policy says no, the tool call is blocked and the reason is returned to the agent. You define the boundaries; the plugin enforces them.

## Prerequisites

- [Codex CLI](https://developers.openai.com/codex/cli) installed and working
- Node.js 18+ (already required by Codex)
...
```bash
codex plugin marketplace add denied-dev/denied-sdk
```
...
On first run, Codex will prompt you to review and trust the plugin's `PreToolUse` hook. Open the hook browser with `/hooks` inside Codex, inspect the definition, and approve it. Codex will not execute non-managed hooks until you do.

### Step 5: Verify it's working

When a tool call is blocked, Codex will display the denial reason inline. You can also check stderr output for lines like:

```
[denied-dev] Blocked tool call: Bash
[denied-dev] Blocked tool call: apply_patch
```
...
The plugin enforces Denied authorization checks on every tool call, **regardless of Codex's approval policy or sandbox mode**. Codex's built-in approval prompts and sandboxing govern _local_ execution; Denied policies enforce _organizational_ boundaries that are orthogonal to local permission delegation.
...
- [Codex CLI](https://developers.openai.com/codex/cli) — OpenAI's agentic coding tool
