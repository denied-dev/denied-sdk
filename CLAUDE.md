...
- **Kiro IDE 1.0.182 is the effective floor, and older builds fail silently.** The v1 JSON hooks system (`"version": "v1"` hooks array, `trigger: "PreToolUse"`, files under `.kiro/hooks/`) shipped in **IDE 1.0.0 on 2026-06-25** (kiro.dev/changelog/ide/, kiro.dev/docs/ide/whats-new-v1/); 0.x builds are not without tool interception entirely (0.9 had Pre/Post Tool Use in the legacy `.kiro.hook` agent-hooks format, which the v1 migration maps `preToolUse` → `PreToolUse` from), but they do not understand the standalone v1 JSON hook file the installer writes and **silently ignore** it — no error, no firing. Builds 1.0.0–1.0.181 have the feature but do not scan user-level global `~/.kiro/hooks/` ([#9075](https://github.com/kirodotdev/Kiro/issues/9075), closed by "Added user-level global hooks" in **1.0.182**, 2026-07-20); only workspace-scoped files load there, so `--workspace=<path>` is the stopgap for that window. Hence the floor of 1.0.182 for the default global install. The Kiro CLI is versioned separately (2.16.x runs V3) and is unaffected. A real support case drove this: an all-`[PASS]` `--check` on IDE 0.12.333 (Windows) with the hook never firing — the probe is sent by the installer itself, not through Kiro, so it proves nothing about the host. Both the plain installer and `--check` therefore detect the installed IDE version from the IDE's on-disk `resources/app/package.json` (macOS `/Applications/Kiro.app/Contents/Resources/app/package.json`, also under `~/Applications`; Windows `%LOCALAPPDATA%\Programs\Kiro` or `%ProgramFiles%\Kiro`; Linux `/usr/share/kiro` or `/opt/Kiro`). `--check` reports a `Kiro IDE version` condition: `[FAIL]` (exit 1) on a detected too-old IDE, with distinct messages for the pre-1.0 "format not understood" and 1.0.0–1.0.181 "global hooks not discovered, use `--workspace`" cases; a **NOTE rather than a failure when no IDE is found**, because a CLI-V3-only machine legitimately has none. Two deliberate refinements keep the verdict honest in both directions: on a 1.0.0–1.0.181 IDE the condition **passes when a workspace hook file was verified in the same run** (`--check --workspace=<path>`), since workspace-scoped hooks do load on those builds and failing the very stopgap the tool recommends would teach users to ignore it — the pass text states that only that workspace is covered; and when the IDE version could **not** be verified, the all-`[PASS]` summary says every prerequisite *that could be verified* is in place and names the IDE version as the gap, rather than claiming every prerequisite was confirmed. The plain installer warns in the same too-old cases rather than blocking, since the install is still correct for CLI V3. Detection sees the installed app, not a running process or a custom install path.
...
- **`KIRO_HOME` does not relocate hooks.** Verified on Kiro CLI 2.16.1 (V3): hooks are read from `~/.kiro/hooks` unconditionally, so the installer writing there is correct. Revisit if [#9148](https://github.com/kirodotdev/Kiro/issues/9148) lands upstream. (#9075 is no longer open — it was the global-hooks *discovery* gap, closed in IDE 1.0.182; see the version-floor bullet above.)
